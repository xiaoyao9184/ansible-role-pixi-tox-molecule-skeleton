...
TOX_ENV_RE = re.compile(
    r"^(?P<tox_env>integration-(?P<py_factor>py\d+\.\d+)-(?P<ansible_factor>\d+\.\d+))$"
)
...
def discover_default_py_envs() -> list[str]:
    ...
    for tox_ini_path in sorted(TOX_DIR.glob("py*/tox-ansible.ini"), key=lambda path: path.parent.name):
...
def collect_tox_envs(py_envs: list[str], ansible_factors: list[str], scenario_name: str) -> list[str]:
    tox_envs: list[str] = []
    wanted_ansible_factors = set(ansible_factors)

    for py_env in py_envs:
        tox_ini_path = TOX_DIR / py_env / "tox-ansible.ini"
        ...
        result = subprocess.run(
            ["pixi", "run", "-e", py_env, "tox-ansible", "-l"],
            check=True,
            cwd=PROJECT_ROOT,
            text=True,
            capture_output=True,
        ...
        for line in result.stdout.splitlines():
            match = TOX_ENV_RE.match(line.strip())
            if match is None:
                continue

            tox_env = match.group("tox_env")
            ansible_factor = match.group("ansible_factor")
            if ansible_factor not in wanted_ansible_factors:
                continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    filter_parser.add_argument(
        "--py-envs",
        help=(
            "Comma-separated or newline-separated pixi Python environments used to run `tox-ansible -l`, "
            "default: discovered from tox/py*/tox-ansible.ini"
        ),
    )
